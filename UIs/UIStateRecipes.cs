...
namespace TRaI.UIs
{
    ...
    public class UIStateRecipes : UIState
    {
        ...
        //Categories
        public List<RecipeCategory> ActiveCategories { get; set; } = new();
        ...
        public List<IRecipeElement> ActiveRecipes { get; set; } = new();
        ...
        //History
        public List<(Mode mode, IIngredient ingredient, int category, int page)> History { get; set; } = new();
        ...
        public override void OnInitialize()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                leftButton = new UIImageButton(Main.Assets.Request<Texture2D>("Images/UI/Bestiary/Button_Back"));
                leftButton.SetHoverImage(Main.Assets.Request<Texture2D>("Images/UI/Bestiary/Button_Border"));
                ...
                rightButton = new UIImageButton(Main.Assets.Request<Texture2D>("Images/UI/Bestiary/Button_Forward"));
                rightButton.SetHoverImage(Main.Assets.Request<Texture2D>("Images/UI/Bestiary/Button_Border"));
            ...
            LeftButtons[0].OnClick += (_, _) => { CurrentCategoryPage--; SoundEngine.PlaySound(SoundID.MenuTick); };
            RightButtons[0].OnClick += (_, _) => { CurrentCategoryPage++; SoundEngine.PlaySound(SoundID.MenuTick); };

            LeftButtons[1].OnClick += (_, _) => { HistoryBack(); SoundEngine.PlaySound(SoundID.MenuTick); };
            RightButtons[1].OnClick += (_, _) => { HistoryForward(); SoundEngine.PlaySound(SoundID.MenuTick); };

            LeftButtons[2].OnClick += (_, _) => { CurrentPage--; SoundEngine.PlaySound(SoundID.MenuTick); };
            RightButtons[2].OnClick += (_, _) => { CurrentPage++; SoundEngine.PlaySound(SoundID.MenuTick); };

            Panels[0].OnClick += (_, _) => { TRaIUI.OpenRecipes(Mode.All); SoundEngine.PlaySound(SoundID.MenuTick); };
            Panels[1].OnClick += (_, _) => { ClearHistory(); SoundEngine.PlaySound(SoundID.MenuTick); };
            Panels[2].OnClick += (_, _) => { CurrentPage = 0; SoundEngine.PlaySound(SoundID.MenuTick); };
            ...
            CategoryTabs[0].OnClick += (_, _) => CurrentCategory = 0;
            CategoryTabs[1].OnClick += (_, _) => CurrentCategory = 1;
            CategoryTabs[2].OnClick += (_, _) => CurrentCategory = 2;
            CategoryTabs[3].OnClick += (_, _) => CurrentCategory = 3;
            CategoryTabs[4].OnClick += (_, _) => CurrentCategory = 4;
            CategoryTabs[5].OnClick += (_, _) => CurrentCategory = 5;
            CategoryTabs[6].OnClick += (_, _) => CurrentCategory = 6;
            CategoryTabs[7].OnClick += (_, _) => CurrentCategory = 7;
            CategoryTabs[8].OnClick += (_, _) => CurrentCategory = 8;
            CategoryTabs[9].OnClick += (_, _) => CurrentCategory = 9;
            CategoryTabs[10].OnClick += (_, _) => CurrentCategory = 10;
            CategoryTabs[11].OnClick += (_, _) => CurrentCategory = 11;
        }
